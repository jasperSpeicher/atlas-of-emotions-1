...
export default {
	...
	setUpGraphs: function (containerNode) {
		...
		let margin = {
			...
			bottom: this.screenIsSmall
				? sassVars.actions.margins["bottom-small"]
				: sassVars.actions.margins.bottom,
			left: this.screenIsSmall ? graphContainer.offsetWidth / 3.7 : 100,
		};

		// All the same size, just grab the first one
		let innerWidth =
				graphContainer.offsetWidth - margin.left - margin.right,
			h = Math.max(
				graphContainer.offsetHeight,
				0.5 * graphContainer.offsetWidth
			),
		...
		this.arcGenerator = d3.svg.arc().innerRadius(0).outerRadius(radius);
		...
		if (!this.graphContainers) {
			...
			_.values(dispatcher.EMOTIONS).forEach((emotion, i) => {
				let graphContainer = document.querySelector(
					"#actions ." + emotion + " .graph-container"
				);
				...
				/*
				 ...
				 .stroke('black');
				 */
				valenceTextures[VALENCES.CONSTRUCTIVE] = textures
					.lines()
					.thinner(0.85)
					.orientation("6/8")
					.stroke("black");
				valenceTextures[VALENCES.BOTH] = textures
					.lines()
					.orientation("4/8")
					.stroke("black");
				valenceTextures[VALENCES.DESTRUCTIVE] = textures
					.lines()
					.thicker(0.85)
					.orientation("2/8")
					.stroke("black");
	...
	parseActions: function () {
		...
		let actionsData = Object.keys(dispatcher.EMOTIONS).reduce(
			(actionsOutput, emotionKey) => {
				let emotionName = dispatcher.EMOTIONS[emotionKey],
					statesData = appStrings().getStr(
						`emotionsData.emotions.${emotionName}.states`
					);

				// copy list of all actions for emotion with lowercased names, and alpha sort
				let allActionsForEmotionRaw = appStrings()
					.getStr(`emotionsData.emotions.${emotionName}.actions`)
					.map((action) => {
						return Object.assign({}, action, {
							name: action.name.toLowerCase(),
						});
					});
				let allActionsForEmotion = allActionsForEmotionRaw
					.concat()
					.sort((a, b) => {
				...
				if (emotionName === dispatcher.EMOTIONS.ANGER) {
					...
					allActionsForEmotion.splice(
						1,
						0,
						allActionsForEmotion.shift()
					);
					allActionsForEmotion.splice(
						-1,
						0,
						allActionsForEmotion.pop()
					);
				...
				let actionsByState = statesData.reduce(
					(statesOutput, state) => {
						...
						let actionsBoth = state.actions.both
								.map(lowercase)
								.filter((action) => {
									let actionName = action;
									if (!isNaN(parseInt(action)))
										actionName =
											allActionsForEmotionRaw[+action - 1]
												.name;
									return !!allActionsForEmotionByName[
										actionName
									];
								})
								.map((action) =>
							...
							allActionsForState = actionsCon
								.concat(actionsBoth)
								.concat(actionsDes),
							numAllActionsForState = allActionsForState.length;

						let actionByName, action;
						allActionsForState = allActionsForState.map(
							(actionName, i) => {
								// copy prototypical action name/description
								actionByName =
									allActionsForEmotionByName[actionName];
								action = {
								...
								action.rotation =
									100 +
									((numAllActionsForState - i - 1) * 160) /
										(numAllActionsForState - 1);
						...
						let valenceWeights = Object.keys(VALENCES)
							.map((valenceName) => ({
								name: valenceName,
								size:
									valenceName === "CONSTRUCTIVE"
										? numActionsCon
										: valenceName === "BOTH"
										? numActionsBoth
										: valenceName === "DESTRUCTIVE"
										? numActionsDes
										: 0,
							}))
							.reverse();
						...
						let firstNonEmptyWeight, lastNonEmptyWeight;
	...
	setEmotion: function (emotion) {
		return new Promise((resolve, reject) => {
			...
			let dx = 0;
			if (previousEmotion) {
				let previousContainer = d3.select(
					".actions-container." + previousEmotion
				);
				...
				if (
					...
					emotions.indexOf(previousEmotion)
				) {
					dx *= -1;
				}
			...
			if (previousEmotion) {
				...
				resolveDelay =
					(sassVars.emotions.panX.delay +
						sassVars.emotions.panX.duration) *
					1000;
			} else {
	...
	setState: function (state) {
		...
		let stateActionsData, currentActionsData;

		if (state) {
			stateActionsData =
				this.actionsData[this.currentEmotion].actions[
					this.currentState
				];
			if (!stateActionsData) {
				console.warn(
					'No actions found for state "' +
						this.currentState +
						'" in emotion "' +
						this.currentEmotion +
						'".'
			...
			currentActionsData =
				this.actionsData[this.currentEmotion].allActions;
		}

		let emotionGradientName =
				"actions-" + this.currentEmotion + "-gradient",
			valenceTextureSet = this.valenceTextures[this.currentEmotion],
			graphContainer = this.graphContainers[this.currentEmotion];

		let arrowSelection = graphContainer
			.selectAll("g.action-arrow")
			.data(currentActionsData, (d) => d.name);

		// update
		arrowSelection.classed({
		...
		// enter
		let arrowEnterSelection = arrowSelection
			.enter()
			.append("g")
		...
		if (!this.isBackgrounded) {
			if (this.screenIsSmall) {
				d3.select(
					`#actions .${this.currentEmotion} .graph-container`
		...
		if (
			...
			~window.navigator.userAgent.toLowerCase().indexOf("gecko") &&
	...
	setHighlightedState: function (state) {
		if (!state) {
			...
			let allActions = this.actionsData[this.currentEmotion].allActions,
				stateActions = this.actionsData[this.currentEmotion].actions[
					state
				].actions.map((action) => action.name),
				otherActions = allActions
					.filter((action) => !~stateActions.indexOf(action.name))
			...
			graphContainer
				...
				.classed(
					"muted",
					(data, index) => !~stateActions.indexOf(data.name)
				);
	...
	setLabelStates: function (labelContainer, highlighted, muted = []) {
		...
		const selected = this.highlightedAction
		...
		if (selected || highlighted.length) {
			labels.classed({
				muted: (d) =>
					(highlighted.indexOf(d.name) < 0 && d.name !== selected) ||
					~muted.indexOf(d.name),
			...
			labelContainer
				.select(".default-interactive-helper")
	...
	renderLabels: function (actionsData, immediate) {
		...
		let labelText = (d) => {
			 ...
			 */
			return (
				'<span class="label-name">' + d.name.toUpperCase() + "</span>"
			);
		};
		...
		if (actionsData) {
					...
					(() =>
						function (d, i, a) {
							let previousTransform =
									a === "none" ? null : d3.transform(a),
								previousTranslate = previousTransform
									? previousTransform.translate
									: [0, 0],
								verticalOffset =
									i === 0 || i === labelSelection.size() - 1
										? 20
										: 0,
								// critical to fully invert function that translates from rotation to x/y translation
								previousRotation = Math.atan2(
									(previousTranslate[1] - verticalOffset) *
										sqrt3,
									previousTranslate[0] +
										0.5 * this.offsetWidth
								),
								targetRotation =
									(Math.PI * (d.rotation - 90)) / 180,
								rot;

							return (t) => {
								rot =
									previousRotation +
									t * (targetRotation - previousRotation);
								return (
									"translate(" +
									Math.round(
										labelSize * Math.cos(rot) -
											0.5 * this.offsetWidth
									) +
									"px," + // center on label width
									Math.round(
										(labelSize / sqrt3) * Math.sin(rot) +
											verticalOffset
									) +
									"px)"
								); // bump down the first and last label
							};
						})()
				);
			labelSelection.select("h4").html(labelText);

			// get a random label to set a box around it
			if (
				!this.randomLabelPositions.hasOwnProperty(this.currentEmotion)
			) {
				this.randomLabelPositions[this.currentEmotion] =
					actionsData[Math.floor(Math.random() * actionsData.length)];
			}

			// enter
			let labelEnterSelection = labelSelection
				.enter()
				.append("div")
				.attr("class", `${this.currentEmotion} label emotion-label`)
				.attr(
					"data-popuptarget",
					(d, i) => `actions${dispatcher.HASH_DELIMITER}${d.name}`
				)
				.style("opacity", 0.0);
			...
			let t = function (d, i) {
				let verticalOffset =
					i === 0 || i === labelEnterSelection.size() - 1 ? 20 : 0;
				return (
					"translate(" +
					Math.round(
						labelSize *
							Math.cos((Math.PI * (d.rotation - 90)) / 180) -
							0.5 * this.offsetWidth
					) +
					"px," + // center on label width
					Math.round(
						(labelSize *
							Math.sin((Math.PI * (d.rotation - 90)) / 180)) /
							sqrt3 +
							verticalOffset
					) +
					"px)"
			...
			labelSelection.classed(
				...
				(d, i) =>
					d.name ===
					this.randomLabelPositions[this.currentEmotion].name
			);
	...
	setBackgrounded: function (val, options) {
		return new Promise((resolve, reject) => {
			...
			this.sectionHitArea.classList[val ? "add" : "remove"](
				"backgrounded"
			);
	...
	paginateElement: function (dir) {
		...
		if (this.highlightedAction) {
			nextIndex =
				actionsData.findIndex(
					(s) => s.name === this.highlightedAction.name
				) + dir;
			nextIndex =
				nextIndex >= actionsData.length
					? 0
					: nextIndex < 0
			...
			if (action.valence) {
				secondaryData = {
					body:
						action.valence &&
						appStrings().getStr(
							`emotionsData.metadata.actions.qualities[${
								action.valence - 1
							}].body`
						),
					classes: [
	...
	displayHighlightedAction: function (action, valence) {
		...
		let highlightedAction = action || this.highlightedAction || null,
			arrowSelection =
				this.graphContainers[this.currentEmotion].selectAll(
					"g.action-arrow"
				),
			labelSelection =
				this.labelContainers[this.currentEmotion].selectAll(
					"div.label"
				);
		...
		if (highlightedAction) {
			if (valence) {
				...
				arrowSelection.classed("muted", (d) => {
					if (valence) {
				...
				arrowSelection.classed(
					"muted",
					(data, index) => data.name !== highlightedAction.name
				);
			}

			this.setLabelStates(this.labelContainers[this.currentEmotion], [
				highlightedAction.name,
			]);
		} else {
			arrowSelection.classed("muted", false);
	...
	onContainerTouchMove: function () {
		...
		let touchedShape = document.elementFromPoint(
		...
		if (touchedShape.nodeName === "path") {
			...
			const angThreshold = Math.min(
					10,
					180 /
						(1.5 *
							this.actionsData[this.currentEmotion].allActions
								.length)
				), // detect touches within angle determined by total number of arrows,
	...
	resetSectionText: function () {
		...
		dispatcher.changeSectionText(
			appStrings().getStr(
				`emotionsData.emotions.${this.currentEmotion}.continent.name`
			),
			...
			appStrings().getStr("emotionsData.metadata.actions.body") +
				...
				"</h3>" +
				appStrings().getStr(
					`emotionsData.emotions.${this.currentEmotion}.actions[0].secondary`
				) +
				"<br/>&nbsp;<br/>" +
				appStrings().getStr(
					"emotionsData.metadata.actions.interaction_prompt"
				)
		);
	},
};
