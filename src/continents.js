...
export default class ContinentsSection {
	...
	init(containerNode, screenIsSmall) {
		...
		this.labelContainer = d3
			.select(containerNode)
			.append("div")
			.attr("class", "label-container");

		this.continentContainer = d3
			.select(containerNode)
			.append("svg")
			.attr("width", "100%")
			.attr("height", "100%");
		...
		this.continents = _.values(dispatcher.EMOTIONS).map(
			(emotion) =>
				new Continent(
					emotion,
		...
		Object.keys(this.transitions).forEach((transitionKey) => {
			this.transitions[transitionKey] =
				this.transitions[transitionKey].bind(this);
		});
	...
	setEmotion(emotion, previousEmotion) {
		return new Promise((resolve, reject) => {
			if (this.currentEmotion) {
				let currentContinent = this.continents.find(
					(c) => c.id === this.currentEmotion
				);
				...
				if (emotion) {
					if (this.zoomedInContinent) {
						...
						if (this.zoomedInContinent !== previousEmotion) {
							...
							this.transitions.gatherContinent(
								this.zoomedInContinent,
								true
							);
						...
						setTimeout(() => {
							...
							// pan to center
							this.transitions.panToContinent(
								null,
								this.currentEmotion
							);
							...
							dispatcher.changeSectionText(
								null,
								appStrings().getStr(
									"emotionsData.metadata.continents.header"
								),
								appStrings().getStr(
									"emotionsData.metadata.continents.body"
								)
							);
						...
						currentContinent.highlightLevel =
							Continent.HIGHLIGHT_LEVELS.UNSELECTED;

						let continent = this.continents.find(
							(c) => c.id === emotion
						);
						this.setContinentHighlight(
							continent,
							Continent.HIGHLIGHT_LEVELS.SELECTED
						);
						dispatcher.changeSectionText(
							null,
							appStrings().getStr(
					...
					this.setContinentHighlight(
						null,
						Continent.HIGHLIGHT_LEVELS.NONE
					);
					...
					if (this.zoomedInContinent) {
						...
						setTimeout(() => {
							let allEmotions = this.continents.map(
								(continent) => continent.id
							);
							this.transitions
								.scaleContinents(allEmotions, 0.0, undefined, 0)
								.then(() => {
									return this.transitions.scaleContinents(
										allEmotions,
										1.0
									);
								})
	...
	getPopupDescription(emotion) {
		return emotion
			? appStrings().getStr(
					`emotionsData.emotions.${emotion}.continent.desc`
			  )
			: null;
	}
	...
	initLabels(labelContainer) {
		...
		const positionLabelsVertically =
			this.positionLabelsVertically.bind(this);
		let labelsEnter = labels
			...
			.classed(
				...
				(d) =>
					d.name.toLowerCase() ===
					this.defaultEmotionHelper.toLowerCase()
			)
			.each(function (d, i) {
	...
	initMobileElements(containerNode, labelContainer) {
		labelContainer
			...
			.append("p")
			.html(
				appStrings().getStr("emotionsData.metadata.intro.body_mobile")
			);
	...
	close(nextSection) {
		return new Promise((resolve, reject) => {
			...
			if (
				nextSection === dispatcher.SECTIONS.STATES &&
				continent &&
				continent.highlightLevel === Continent.HIGHLIGHT_LEVELS.SELECTED
			) {
				...
				let targetScale = 1.0,
					...
					spreadDuration =
						sassVars.continents.spread.duration.in * 1000;
				...
				setTimeout(() => {
					targetScale = this.transitions.focusZoomedOutContinent(
						continent.id
					);
				}, spreadDelay);

				setTimeout(() => {
					this.transitions.spreadFocusedContinent(
						continent.id,
						targetScale
					);
				}, spreadDelay);
	...
	onResize(screenIsSmall) {
		...
		this.continents.forEach((c) =>
			c.onResize(continentGeom, this.screenIsSmall)
		);

		// update label positions
		let labels = this.labelContainer
			.selectAll(".emotion-label")
			.data(this.continents, (d) => d.id);
		...
		const positionLabelsVertically =
			this.positionLabelsVertically.bind(this);
		const positionLabelsHorizontally =
			this.positionLabelsHorizontally.bind(this);
	...
	setActive(val) {
		...
		this.labelContainer.selectAll(".emotion-label").classed("visible", val);
	}
	...
	setInteractive(val) {
		...
		d3.select("#main").on(
			"click",
			val ? this.onBackgroundClick : null,
			false
		);
	}
	...
	setMobileIntroVisibility(val) {
		this.labelContainer
			.select(".intro-element.message")
			.classed("visible", val);

		let introButton = d3
			.select(".intro-element.button")
			.style("display", "block");
	...
	update(time) {
		...
		let updateState = {
			time: time,
			someContinentIsHighlighted: this.continents.some(function (
				continent
			) {
		...
		this.continents.forEach((continent) =>
			continent.update(updateState, this.frameCount)
		);
	...
	transitions = {
		...
		focusZoomedOutContinent: function (emotion) {
			let targetContinent = this.continents.find(
					(continent) => continent.id === emotion
				),
				targetScale =
					(0.45 *
						this.continentContainer.node().getBoundingClientRect()
							.height) /
					targetContinent.size;
		...
		panToContinent: function (emotion, previousEmotion, immediate) {
			...
			let statesBaselineOffset =
				this.centerY -
				(this.sectionContainer.offsetHeight *
					(parseInt(
						sassVars.states.containers.bottom.replace("%", "")
					) /
						100) +
					50);
			...
			let targetContinent = this.continents.find(
				...
				previousContinent = this.continents.find(
					(continent) => continent.id === previousEmotion
				),
				targetCenter = {
					x:
						this.centerX -
						(previousContinent ? previousContinent.x : 0),
					y:
						this.centerY -
						(previousContinent
							? previousContinent.y - statesBaselineOffset
							: 0),
				},
			...
			let durationX, durationY, funcX, funcY;
		...
		spreadFocusedContinent: function (emotion, targetScale) {
			...
			targetContinent.spreadCircles(
				this.continentContainer.node(),
				targetScale
			);
		},
		...
		scaleContinents: function (emotions, scale, delays = {}, time = 1200) {
			...
			return new Promise((resolve, reject) => {
				let targetContinents = this.continents.filter(
					(continent) => ~emotions.indexOf(continent.id)
				);
	...
	onContinentMouseEnter(continent) {
		...
		this.setContinentHighlight(
			continent,
			Continent.HIGHLIGHT_LEVELS.HIGHLIGHTED
		);
	...
	unsetContinentHighlight(continent) {
		...
		if (otherHighlightedContinent) {
			...
			let unhighlightLevel =
				otherHighlightedContinent.highlightLevel ===
				Continent.HIGHLIGHT_LEVELS.SELECTED
					? Continent.HIGHLIGHT_LEVELS.UNSELECTED
					: Continent.HIGHLIGHT_LEVELS.UNHIGHLIGHTED;
			if (
				continent &&
				continent.highlightLevel !== Continent.HIGHLIGHT_LEVELS.SELECTED
			) {
	...
	setContinentHighlight(continent, highlightLevel) {
		...
		let unhighlightLevel;
		if (
			highlightLevel === Continent.HIGHLIGHT_LEVELS.SELECTED ||
			this.continents.some(
				(c) => c.highlightLevel === Continent.HIGHLIGHT_LEVELS.SELECTED
			)
		) {
	...
	setLabelStates() {
		...
		const somethingSelected = this.continents.filter(
			(c) => c.highlightLevel === Continent.HIGHLIGHT_LEVELS.SELECTED
		);

		const somethingHighlighted = this.continents.filter(
			(c) => c.highlightLevel === Continent.HIGHLIGHT_LEVELS.HIGHLIGHTED
		);
	...
	positionLabelsVertically(d, i, target) {
		...
		if (bottomness > 1.5) {
			...
			target.style.bottom =
				-Math.round(this.centerY + d.y + d.label.y) + "px";
		} else {
	...
	positionLabelsHorizontally(d, i, target) {
		...
		if (
			sectionTextRect.width + left + textRect.width / 2 >
			window.innerWidth
		) {
			left =
				window.innerWidth - textRect.width / 2 - sectionTextRect.width;
		}
