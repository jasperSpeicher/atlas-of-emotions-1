...
import * as logo from "./atlas-logo.svg";
...
export default function (...initArgs) {
		...
		appStrings()
			...
			.then(() => {
				...
				dispatcher.addListener(
					dispatcher.EVENTS.CHANGE_EMOTION_STATE,
					onEmotionStateChange
				);
				dispatcher.addListener(
					dispatcher.EVENTS.CHANGE_EMOTION,
					onEmotionChange
				);
				dispatcher.addListener(
					dispatcher.EVENTS.CHANGE_SECTION_TEXT,
					onSectionTextChange
				);
				dispatcher.addListener(
					dispatcher.EVENTS.POPUP_CHANGE,
					onPopupChange
				);

				// other events
				dispatcher.addListener(
					dispatcher.EVENTS.SECTION_GRAPHICS_RESIZE,
					onSectionGraphicsResized
				);
				dispatcher.addListener(
					dispatcher.EVENTS.SECTION_TEXT_MAXIMIZE_START,
					onSectionTextMaximizeStart
				);
				dispatcher.addListener(
					dispatcher.EVENTS.SECTION_TEXT_MAXIMIZE_COMPLETE,
					onSectionTextMaximizeComplete
				);
				dispatcher.addListener(
					dispatcher.EVENTS.SECTION_TEXT_MINIMIZE_START,
					onSectionTextMinimizeStart
				);
				dispatcher.addListener(
				...
				dispatcher.addListener(
					dispatcher.EVENTS.MAXIMIZE_SECTION_TEXT,
					onMaximizeSectionText
				);
				dispatcher.addListener(
	...
	function initTemplate() {
		...
		let widowFixElements = [].slice.call(
			document.querySelectorAll("[data-widow-fix]")
		);
	...
	function initLanguageSelector() {
		...
		for (let selector of selectors) {
			...
			let dropdown = document.querySelector(selector + " .dropdown"),
				...
				langFile = getActiveLanguages().find(
					(f) => f.lang === getLanguagePref()
				);
			...
			dropdown
				...
				.addEventListener("click", onLangMenuClick);
		}
	}
	...
	function setSectionEmotion(section, previousEmotion, previousMorePage) {
		let promise = section.setEmotion(
			currentEmotion,
			previousEmotion,
			currentMorePage,
			previousMorePage
	...
	function setSection(sectionName, previousEmotion, previousMorePage) {
		...
		if (!previousSection) {
			...
			for (let key in containers) {
				if (
					key !== sectionName &&
					!~backgroundSections.indexOf(sections[key])
				) {
			...
			if (previousSection === section) {
				...
				let previousBackgroundSections =
					previousSection.backgroundSections || [];
				if (!previousSectionBackgrounded) {
					// don't mess with current backgroundSections or the current section
					previousBackgroundSections =
						previousBackgroundSections.filter((prevBkgdSection) => {
							return (
								prevBkgdSection !== section &&
					...
					if (previousBackgroundSections.length) {
						...
						previousBackgroundSections.forEach(
							(prevBkgdSection) => {
				...
				Promise.all(promises).then((values) => {
					...
					if (previousBackgroundSections.length) {
						for (let key in sections) {
							if (
								~previousBackgroundSections.indexOf(
									sections[key]
								) &&
								(!section.backgroundSections ||
									!~section.backgroundSections.indexOf(
										sections[key]
									))
							) {
								containers[key].style.display = "none";
	...
	function onResize() {
		...
		if (!screenIsSmall) {
			let headerHeight = screenIsSmall
				? sassVars.ui.header["height-small"]
				: sassVars.ui.header.height;
			document.getElementById("main").style.height =
				window.innerHeight - headerHeight + "px";
		}

		// update all sections
		let section, sectionContainer, containerIsHidden;
	...
	function onLangMenuItemClick(event) {
		...
		document
			.querySelector("#lang-selector .dropdown")
	...
	function onPopupChange(section, emotionState, desc, secondaryData) {
		...
		if (!section) {
			...
			if (
				emotionState !== popupManager.currentName ||
	...
	function adjustForScreenSize() {
		...
		if (
			bypassedWarning ||
			/*probablyDesktop || */ (window.innerWidth >= MIN_ALLOWED_WIDTH &&
				window.innerHeight >= MIN_ALLOWED_HEIGHT)
		) {
			//if ( MOBILE_ENABLED ) {
	...
	function getLanguagePref() {
		...
		if (lang && !getActiveLanguages().find((f) => f.lang === lang))
			lang = null;
