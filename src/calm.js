...
export default {
	...
	init: function (containerNode, screenIsSmall) {
		...
		this.elapsedToOpacity = d3.scale
			.linear()
			.domain([0, 0.05, 0.25, 0.4, 1])
			.range([0, 0.1, 0.9, 0.9, 0]);
		...
		continents = _.values(dispatcher.EMOTIONS).map(
			(emotion) =>
				new Continent(
					emotion,
					continentContainer,
		...
		dispatcher.addListener(
			dispatcher.EVENTS.POPUP_CLOSE_BUTTON_CLICKED,
			this.onPopupCloseButtonClicked.bind(this)
		);
	...
	initLabels: function (labelContainer) {
		continents.forEach((c) => {
			...
			let labelsEnter = labels
				...
				.attr(
					"class",
					(d) => `emotion-label label show-only no-body ${d.id}`
				)
				.attr(
					"data-popuptarget",
					(d) => `calm${dispatcher.HASH_DELIMITER}${d.id}`
				)
				.attr("data-clip", "1")
			...
			labelsEnter
				...
				.attr(
					"href",
					(d) => `#continents${dispatcher.HASH_DELIMITER}${d.id}`
				)
				.append("h3")
				.text(
					appStrings().getStr(`emotionsData.metadata.calm.caption`) ||
						"START OVER"
				);
		});
	},
	...
	spawnNew: function (currentState, i) {
		...
		currentState.duration[i] =
			currentState.durationBase * Math.random() +
			currentState.durationSpread;
	},
	...
	update: function (time) {
		...
		for (var i = 0; i < 5; i++) {
			...
			if (
				...
				if (
					(rand < 0.0015 &&
						numActive < currentState.maxNumVisible - 1) ||
					(rand < 0.0001 && numActive < currentState.maxNumVisible) ||
					numActive == 0
				) {
			...
			if (
				currentState.activeEmotion[i] == 1 &&
				currentState.start[i] + currentState.duration[i] < frameCount
			) {
				currentState.activeEmotion[i] = 0;
			...
			if (currentState.activeEmotion[i] != 0) {
				currentState.opacity[i] = this.elapsedToOpacity(
					this.calcPercentElapsed(
						frameCount,
		...
		continents.forEach(function (continent, i) {
			continent.d3Selection
				...
				.style("display", () =>
					currentState.opacity[i] == 0 ? "none" : "block"
				);
		});
