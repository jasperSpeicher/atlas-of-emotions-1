	...
	gsap.globalTimeline
		...
		.forEach((tween) =>
			tween
				.targets()
				.forEach(
					(e) => e.nodeType && ancestor.contains(e) && tween.kill(e)
				)
		);
}
export default class Episode {
	...
	setEmotion(emotion) {
		let animateReplaceContent =
			this.currentEmotion != emotion && !this.playFromStart;

		this.currentEmotion =
			emotion && emotion != "" ? emotion : dispatcher.DEFAULT_EMOTION;

		if (this.replayEnabled) {
			if (this.playFromStart) {
				TweenMax.delayedCall(1.5, this.start.bind(this), [
					animateReplaceContent,
				]);
			} else {
				this.rewind(() => {
	...
	replaceContent(emotion, animate = false) {
		...
		this.statePhraseText.forEach(
			this.replaceTextContentForKey("phrases", "common", false)
		);
		this.stateNameText.forEach(
			this.replaceTextContentForKey("state", emotion, false)
		);
		this.responseText.forEach(
			this.replaceTextContentForKey("response", emotion, false)
		);
		this.triggerText.forEach((child, i) => {
			let animateText = animate && child.parentNode.id != "event-text";
			let replace = this.replaceTextContentForKey(
				"trigger",
				emotion,
				animateText
			);
	...
	getScrollElements() {
		if (!this.scrollElements) {
			this.scrollElements = [this.parent.querySelector("svg")];
		}
	...
	touchDeflect(distance) {
		if (this.getInteractive()) {
			...
			TweenMax.set(this.getScrollElements(), {
				css: { left: this.svgScrollPosition + this.touchDeflection },
			});
		}
	}
	...
	returnTouchDeflection() {
		if (this.getInteractive()) {
			this.scrollTween = TweenMax.to(this.getScrollElements(), 0.4, {
				css: { left: this.svgScrollPosition },
			});
	...
	scrollSvgInDirection(direction) {
		if (this.getInteractive()) {
			...
			if (
				scrollStageIndex >= 0 &&
				scrollStageIndex < this.scrollStages.length
			) {
	...
	computeScrollPositions() {
		...
		this.scrollStages.forEach((stage) => {
			this.scrollStagePositions[stage] =
				-1 *
				(this.getStageDOMCenterPoint(stage) -
		...
		if (
			...
			if (this.minimizing || !this.maximized) {
				...
				this.scrollTween = TweenMax.to(
					...
					duration,
					{ css: { left: this.scrollStagePositions[stage] } }
				);
			}
		}
	}
	...
	minimizeStart(duration) {
		...
		if (this.scrollTween && this.scrollTween.totalProgress() < 1) {
		...
		TweenMax.fromTo(
			...
			duration,
			{
				css: {
					...
					left: this.scrollStagePositions[
						this.scrollStages[this.scrollStageIndex]
					],
				},
			},
			{
	...
	extractDocument(svgElement, container) {
		...
		wrapperGroup.setAttribute(
			"transform",
			"translate(" +
				-0.5 * parseInt(originalWidth) +
				", " +
				-0.5 * parseInt(originalHeight) +
				")"
		);
	...
	constructor(svg, container, emotion, screenIsSmall) {
		...
		this.scrollStages = ["trigger", "experience", "response"];
	...
	initialize(svg, container, emotion, screenIsSmall) {
		if (svg) {
			...
			this.content = _.mapValues(
				timeline.episodeContent,
				function (value, key) {
					if (key === "common") {
						return value;
					} else {
						return _.mapValues(value, (innerValue) => [
							innerValue[Object.keys(innerValue)[1]],
						]);
					}
				}
			);
			...
			gsap.globalTimeline.to(
				timeline.getChildren(timeline.select("#state", this.parent)),
				0,
				{ visibility: "hidden" }
			);
			...
			this.triggerText = [timeline.select("tspan", event)];

			d3.selectAll(this.triggerText)
				.attr("text-anchor", "middle")
				.attr("x", function () {
					return (
						parseFloat(this.getComputedTextLength()) / 2 +
						parseFloat(this.getAttribute("x"))
					);
				});

			this.statePhraseText = [stateLabelChildren[0]];
			this.stateNameText = [stateLabelChildren[1]];

			this.responseText = [responseChildren[0]];
	...
	fixEventLineOverlap(event, eventLine, timeline) {
		...
		if (textLineSpace < minGap) {
			...
			eventLine.setAttribute(
				"d",
				path.replace(/M[^,]+,/, `M${minGap.toString()},`)
			);
		}
	}
}
