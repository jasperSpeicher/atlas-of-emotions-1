...
export default class BlockDiagram {
	...
	getCaptionForElement(element) {
		return timeline.select(
			"#" + element.getAttribute("id") + "-caption",
			timeline.sectionContainer
		);
	}

	getActiveCaption() {
		return timeline.select(
			".diagram-caption.active",
			timeline.sectionContainer
		);
	}
	...
	toggleElement(element) {
		...
		if (block.style.visibility == "hidden") {
			gsap.to(block, { duration, autoAlpha: 1, ease: "power2.out" });
			gsap.to(element, { duration, autoAlpha: 0, ease: "power2.out" });
		} else {
			gsap.to(block, { duration, autoAlpha: 0, ease: "power2.out" });
			gsap.to(element, { duration, autoAlpha: 1, ease: "power2.out" });
		}
	...
	addHoverHandler(element) {
		...
		this.getBlockForElement(element).addEventListener(
			"mouseover",
			function () {
				this.mouseOverHandler.call(this, element);
			}.bind(this)
	...
	constructor(element, parent) {
		...
		for (let i = 0; i < this.blocks.length; i++) {
			...
			let label = appStrings().getStr(
				`emotionsData.metadata.timeline.secondary.blocks.labels.${blockAppString}`
			);
