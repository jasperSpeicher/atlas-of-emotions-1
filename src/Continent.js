...
export default class Continent {
	...
	static HIGHLIGHT_ALPHA_MODS = [1.0, 0.25, 0.5, 1.5, 2.0];
	static HIGHLIGHT_SPEED_MODS = [1.0, 0.25, 0.5, 1.5, 2.0];
	...
	initInstanceProperties(emotion, container, transforms) {
		let emotionIndex = Object.keys(Continent.configsByEmotion).indexOf(
			emotion
		);
		if (emotionIndex === -1) {
		...
		(this.id = emotion),
			...
			(this.i18nName = appStrings().getStr(
				`derived.emotions.${emotion}`
			));
		this.colorPalette =
			Continent.configsByEmotion[emotion].colorPalette.concat();

		this.spawnConfig = {
			lastSpawn: 0,
			minDelay: (this.screenIsSmall ? 2 : 3) * FRAMERATE,
			freq: this.screenIsSmall ? 0.05 : 0.01,
		};
		...
		this.baseTransforms = Object.assign(
			{},
			Continent.transforms[emotionIndex],
			transforms[emotionIndex]
		);
		...
		this.circleWrapper = this.d3Selection
			.append("g")
	...
	prepopulate() {
		let numCircles =
			0.333 * this.maxNumCircles +
			Math.floor(Math.random() * 0.666 * this.maxNumCircles);
	...
	update(state, frameCount) {
		...
		let newCircle, circle, alphaMod, speedMod, scale;
	...
	onResize(continentGeom, screenIsSmall) {
		...
		this.screenIsSmall = screenIsSmall;
		this.maxNumCircles = this.screenIsSmall
			? MAX_NUM_CIRCLES_SMALL
			: MAX_NUM_CIRCLES;

		if (!this.circleHitArea && this.screenIsSmall) {
			this.circleHitArea = this.d3Selection
	...
	spreadCircles(container, containerScale) {
		...
		let margin = {
			...
			statesGraphWidth =
				parseFloat(sassVars.states.graph.width.replace("%", "")) / 100,
			containerBounds = container.getBoundingClientRect(),
			innerWidth =
				(statesGraphWidth * containerBounds.width -
					margin.left -
					margin.right) /
				containerScale,
			centerOffset =
				((sassVars.continents.centerX - 0.5) * containerBounds.width) /
				containerScale,
			xScale = d3.scale
				.linear()
				.domain([0, 10])
				.range([
					-0.5 * innerWidth - centerOffset,
					0.5 * innerWidth - centerOffset,
				]),
			rScale = d3.scale.linear().domain([0, 10]).range([0, innerWidth]);

		const growTime = sassVars.continents.spread.duration.in * 1000;

		let ranges = this.transformRanges(
				emotionsData.emotions[this.id].states
			),
			circles = this.circleWrapper
				.selectAll("circle")
				.data(ranges, (d, i) => i);

		let calcStrokeColor = function (d, i) {
			let colorPalette =
					Continent.configsByEmotion[continent.id].colorPalette,
				color =
					colorPalette[
	...
	gatherCircles(immediate) {
		const growTime = immediate
			? 0
			: sassVars.continents.spread.duration.out * 1000;
		...
		let calledOnEnd = false,
			circles = this.circleWrapper.selectAll("circle").data(this.circles);
