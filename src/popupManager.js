...
class PopupManager {
	...
	open(props) {
		...
		clone.querySelector(".body").textContent = props.desc;

		clone
			.querySelector(".close")
			.addEventListener(
				"click",
				this.onPopupCloseButtonClick.bind(this, props.id),
				false
			);
		...
		let w =
			props.target.offsetWidth + sassVars.ui.popups["expansion-amount"];
		...
		if (props.secondaryData) {
			...
			if (props.secondaryData.classes) {
				secondaryContainer.classList.add(
					...props.secondaryData.classes
				);
			}
	...
	close(key) {
		...
		let closePopup = () => {
			popup.addEventListener(
				"transitionend",
				this.onTransitionOut.bind(this, key),
				false
			);
	...
	getTarget(selector) {
		return document.querySelector(
			`.active [data-popuptarget="${selector}"]`
		);
	}
